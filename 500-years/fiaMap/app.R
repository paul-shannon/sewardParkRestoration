...
MapApp = R6Class("MapAppClass",
    ...
    public = list(
       ...
       initialize = function(){
          ...
          #printf("--- private$tbl")
          #print(private$tbl)
          private$tbl <- read.table("trees.tsv", sep="\t", header=TRUE, as.is=TRUE)
          radii <- 3 *  as.integer(0.5 + private$tbl$dbh/2)
          print(radii)
          private$tbl$radius <- radii
          print(private$tbl)
          for (r in 1:nrow(private$tbl)){
             lat = private$tbl[r, "lon"]
             lon = private$tbl[r, "lat"]
             radius = private$tbl[r, "radius"]
             printf("adding tree %d at of radius %d, at %f %f",
                    r, radius, lat, lon)
             label = sprintf("tree %d", r)
          ...
          print("---- querying markers:")
          print(getMapData(private$map))
          if(is.data.frame(private$tbl.regions)){
             printf("adding %d regions", nrow(private$tbl.regions))
             for(r in seq_len(nrow(private$tbl.regions))){
                 ...
                 private$map <- addPolygons(private$map,
                                            ...
                                            color="black", #private$tbl.regions$borderColor[r],
                                            stroke = TRUE,
                                            label=label,
                                            layerId=id,
                                            group=group,
                                            weight=2, #borderWidth,
                                            fillOpacity=opacity,
                                            labelOptions=tooltip.labelOptions)
                } # for r
       ...
       server = function(input, output, session){
         ...
         observeEvent(input$groupsSelector, ignoreInit=TRUE, ignoreNULL=FALSE,  {
            ...
            printf("--- about to show groups: %s", paste(current.groups))
