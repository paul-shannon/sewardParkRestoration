...
if(Sys.getenv("localConfig") == "TRUE"){
   ...
   config.file <- "fiaSites.yaml"
   regions.yaml.file <- "fiaRegions.yaml"
} else {
   localConfig <- FALSE
   config.file <- "https://pshannon.net/sewardParkRestoration/config/fiaSites.yaml"
   regions.yaml.file <- "https://pshannon.net/sewardParkRestoration/config/fiaRegions.yaml"
   }
    ...
    public = list(
       ...
       initialize = function(){
          ...
          private$map <- with(private$tbl, addCircleMarkers(private$map,
                                                            ...
                                                            weight=1,
                                                            fillOpacity=1,
                                                            #popup=sprintf("%s<br>dbh: ", group),
                                                            popup=sprintf("%s<br>dbh: %5.1f", group, dbh),
                                                            radius=radius,
                                                            layerId=name,
                                                            group=group,
          ...
          fluidPage(
             ...
             sidebarLayout(
               sidebarPanel(
                   ...
                   br(), br(),
                   checkboxGroupInput("groupsSelector", "",
               ...
               mainPanel(
                  ...
                  tabsetPanel(type="tabs", id="mapTabs",
                              ...
                              tabPanel(title="Table", value="tableTab", div(id="table"))),
                  width=10
                  ) # mainPanel
               ) # sidebarLayout
            ) # fluidPage
...
deploy <- function()
{
   ...
   deployApp(account="paulshannon", appName="sewardFIA",
