...
//--------------------------------------------------------------------------------
function calculateWoodpeckerColor(tree){
    // R:  colorRampPalette(c("green", "gray", "black"))(6)

    var colors = ["#FFFFFF","#E5E5E5","#CBCBCB","#979797","#4B4B4B","#000000"]
    var damageScore = tree["wpDamage"]
    var breaks = [1,2,3,4,5,6]
    for(let i=0; i < breaks.length; i++){
       if(damageScore < breaks[i]){
          return(colors[i])
          }
       } // for i
    } // calculateWoodpeckerColor
  ...
  window.trees.forEach(function(tree){
      ...
      if(size < 5){
       ...
       const icon = {path: google.maps.SymbolPath.CIRCLE,
                     ...
                     fillOpacity: 1,
                     strokeWeight: 1,
                     fillColor: calculateWoodpeckerColor(tree),
                     strokeColor: '#000',
                     }
       ...
       let infoWindow = new google.maps.InfoWindow({
           content: "<h4> tree #" + tree["id"] + " (" + tree["observer"] + ")</h4>" +
             "<ul>" +
               ...
               "<li> h2: " + tree["h2"] +
               "<li> h3: " + tree["h3"] +
               "<li> woodpecker damage: " + tree["wpDamage"] +
